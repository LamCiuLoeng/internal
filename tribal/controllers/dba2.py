...
class DBA2Controller(BaseController):
    #Uncomment this line if your controller requires an authenticated user
    allow_only=authorize.not_anonymous()

    @expose('tribal.templates.dba.index2')
    @paginate('collections', items_per_page = 20)
    @tabFocus(tab_type = "main")
    def index(self, **kw):
        ####category: PLAYTEX / WONDERBRA / SHOCK ABSORBER
        kw.setdefault('category_id', returnCatId('PLAYTEX / WONDERBRA / SHOCK ABSORBER'))
        #####
        add_items=[]
        id_list = []
        input_add_qty_list = {}
        ...
        if kw.get('ids', ''):
            ...
            if not in_group("Admin") and not in_group("DBA_AE"):
                day = int(dt.now().day)
                if day < 5 or day > 20:
                    flash('Only allowed to place the orders between 5th–20th each month!', "warn")
                    redirect("/dba2/index")
            """
    ...
    @expose('tribal.templates.dba.order_form_update2')
    @paginate('collections', items_per_page = 20)
    @tabFocus(tab_type = "main")
    def updateOrder(self, **kw):
        ...
        if not flag:
            ...
            redirect("/dba2/search")

        """
        if not in_group("Admin") and not in_group("DBA_AE"):
            day = int(dt.now().day)
            if day < 5 or day > 20:
                flash('Only allowed to revise the orders between 5th–20th each month!', "warn")
                redirect("/dba2/viewOrder?code=%s" % kw.get("code", ""))
        """
    ...
    def save_updateOrder(self, **kw):
        ...
        try:
            ...
            redirect("/dba2/viewOrder?code=%s"%rpacEncrypt(order.id))
        redirect("/dba2/index")
    ...
    def cancelOrder(self, **kw):
        try:
            ...
            redirect("/dba2/updateOrder?a=u&code=%s"%rpacEncrypt(kw.get("orderId", "")))
        else:
    ...
    @expose('tribal.templates.dba.order_form_view2')
    ...
    @expose('tribal.templates.dba.search2')
    @paginate('collections', items_per_page = 20)
    @tabFocus(tab_type = "view")
    def search(self, **kw):
        try:
    ...
    def _emailOrderCompleted(self, po, send_to):
        ...
        text=["Dear Customers,", "\n",
              ...
              "We will send the PI to you between 21th and 22th. Please kindly sign it back upon receipt." , "\n",
              "You can revise the order quantity between 5th and 20th.", "\n",
    ...
    def save_customer_item(self, **kw):
        ...
        redirect("/dba2/customer_item")

    #===========================================================================
    # add by cl
    #===========================================================================
    @expose("tribal.templates.dba.report2")
