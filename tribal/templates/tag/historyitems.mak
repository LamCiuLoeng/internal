...
<div class="nav-tree">TAG&nbsp;&nbsp;&gt;&nbsp;&nbsp;History Items</div>

<div style="width:1300px;margin:0px;">
	<div style="overflow:hidden;margin:5px 0px 5px 0px">
	<form id="search_form" name="DataTable" class="tableform" method="post" action="/tag/historyitems">
	...
	<table width="90%" class="gridTable" cellpadding="0" cellspacing="0" border="0">
		<%
			label_attrs = [('PO#',"60"), ("Style","30"),('SO NO',"110"), ('SO Remark',"100"), \
                        ('TAG NO',"30"), ('PO UNITS',"30"), ("Brand","20"), ('Attachment Set',"70"), ('EDI File Version',"60")]
		...
		<tbody>
			...
			%endif
            <%
               ids = returnIds(item[0], item[1], item[2],item[3],item[4],item[6],item[7],item[8],latest=1)
               format = returnFormat(item[6], item[4])
            %>
            <td>
            	%if in_group('Admin'):
            	<input type="checkbox" name="ids" value="${ids}" alt="${item[6]|b}" ref="${format}" />
            	%else:
				&nbsp;
				%endif
            </td>
