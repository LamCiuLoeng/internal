...
<div class="nav-tree">Main&nbsp;&nbsp;&gt;&nbsp;&nbsp;Structural/Flexible Development&nbsp;&nbsp;&gt;&nbsp;&nbsp;To Do List</div>
      ...
      <table cellspacing="0" cellpadding="0" border="0" id="dataTable" class="gridTable">
		...
		<tbody>
	    	...
	    	%if index %2 == 0 :
  			...
  			%endif
  				%if statusType==-10:
  				<td style="border-left:1px solid #ccc"><a href="/sample/updateRequest?id=${c.id}">${str(c)}</a></td>
  				%else:
  				<td style="border-left:1px solid #ccc"><a href="/sample/viewRequest?id=${c.id}">${str(c)}</a></td>
  				%endif
  				<td>&nbsp;${c.project_own}</td>
  				<td>&nbsp;${c.customer}</td>
  				<td>&nbsp;${c.program}</td>
            ...
            <div class="subform-action">
				      <p>				        
				        %if statusType == -1:
                      <input type="button" action_type="A" value="Approve" onclick="ajaxMark('subform_${c.id}','A')"/>&nbsp;
                      <input type="button" action_type="D" value="Disapprove" onclick="ajaxMark('subform_${c.id}','D')"/>
                %elif statusType == 0:
                ...
                %elif status == -2:
                    <input type="button" action_type="R" value="Request Approval" onclick="ajaxMark('subform_${c.id}','R')"/>
                %elif statusType == 3:
                    <input type="button" action_type="G" value="Continue" onclick="ajaxMark('subform_${c.id}','G')"/>
                %endif
				      </p>              
            </div>
				  </td>
