...
<script language="JavaScript" type="text/javascript">
	//<![CDATA[
		...
		function changeProject(program_id){
			...
			if(program_id){
				$.getJSON("/sample/ajaxProjectInfo",
					  	...
					  	if(req.flag == "1"){
							//alert("Error when loading the to-do list.");
  							...
  							for(var i=0;i<req["draft"].length;i++){
  								$("#todo-draft").append('<li class="every-request"><a href="/sample/updateRequest?id='+req["draft"][i][0]+'">'+req["draft"][i][1]+'</a></li>');
  							}
  							
  							$("#todo-new-count").text(req["new_count"]);
  							$("#todo-new .every-request").remove();
  							for(var i=0;i<req["new"].length;i++){
  								$("#todo-new").append('<li class="every-request"><a href="/sample/viewRequest?id='+req["new"][i][0]+'">'+req["new"][i][1]+'</a></li>');
  							}
  							
  							$("#todo-wait-count").text(req["wait_count"]);
  							$("#todo-wait .every-request").remove();
  							for(var i=0;i<req["wait"].length;i++){
  								$("#todo-wait").append('<li class="every-request"><a href="/sample/viewRequest?id='+req["wait"][i][0]+'">'+req["wait"][i][1]+'</a></li>');
							}
						
							$("#todo-cancel-count").text(req["cancel_count"]);
							$("#todo-cancel .every-request").remove();
							for(var i=0;i<req["cancel"].length;i++){
								$("#todo-cancel").append('<li class="every-request"><a href="/sample/viewRequest?id='+req["cancel"][i][0]+'">'+req["cancel"][i][1]+'</a></li>');
							}
							
							$("#todo-pending-count").text(req["pending_count"]);
				              $("#todo-pending .every-request").remove();
				              for(var i=0;i<req["pending"].length;i++){
				                $("#todo-pending").append('<li class="every-request"><a href="/sample/viewRequest?id='+req["pending"][i][0]+'">'+req["pending"][i][1]+'</a></li>');
				              }
						    
						    $("#todo-dev-count").text(req["dev_count"]);
				              $("#todo-dev .every-request").remove();
				              for(var i=0;i<req["dev"].length;i++){
				                $("#todo-dev").append('<li class="every-request"><a href="/sample/viewRequest?id='+req["dev"][i][0]+'">'+req["dev"][i][1]+'</a></li>');
				              }
...
<div id="function-menu">
  ...
  <tbody><tr>
  	...
  	<td width="64" valign="top" align="left"><a href="/sample/newRequest"><img src="/images/images/menu_new_g.jpg"/></a></td>
  	%if in_group('Admin'):
  	    <td width="64" valign="top" align="left"><a href="/sample/combine"><img src="/images/images/combine_g.jpg"/></a></td>
  	%endif
...
<div class="div-todolist">
  <table width="180" cellspacing="0" cellpadding="0" border="0">
    ...
    <tr>
      <td>
	  	<ol id="todo-draft">
	  	  <li><a href="/sample/viewToDoList?statusType=-10">View All Request</a></li>
	  	</ol>
	  </td>
    </tr>
    ...
    <tr>
      <td>
	  	<ol id="todo-new">
	  	  <li><a href="/sample/viewToDoList?statusType=0">View All Request</a></li>
	  	</ol>
	  </td>
    </tr>
    <tr>
      <td height="30" align="center" class="todoliset-menu" background="/images/TDLBG.jpg">Wait for Approval(<span id="todo-wait-count"></span>)</td>
    </tr>
    <tr>
      <td>
	  	<ol id="todo-wait">
	  	  <li><a href="/sample/viewToDoList?statusType=-1">View All Request</a></li>
	  	</ol>
	  </td>
    </tr>
    <tr>
      <td height="30" align="center" class="todoliset-menu" background="/images/TDLBG.jpg">Pending(<span id="todo-pending-count"></span>)</td>
    </tr>
    <tr>
      <td>
      <ol id="todo-pending">
        <li><a href="/sample/viewToDoList?statusType=3">View All Request</a></li>
      </ol>
    </td>
    </tr>
    <tr>
    ...
    <tr>
      <td>
	  	<ol id="todo-cancel">
	  	  <li><a href="/sample/viewToDoList?statusType=-9">View All Request</a></li>
	  	</ol>
	  </td>
    </tr>
    <tr>
    ...
    <tr>
      <td>
	  	<ol id="todo-dev">
	  	  <li><a href="/sample/viewToDoList?statusType=2">View All Request</a></li>
	  	</ol>
	  </td>
    </tr>
  </tbody>
...
<div style="width:1200px;float:left">
	<div style="overflow:hidden;margin:5px 0px 10px 10px">			
		${widget(value=values,action="/sample/index")|n}
	</div>
      ...
      <table cellspacing="0" cellpadding="0" border="0" id="dataTable" class="gridTable">
		<thead>
			...
			<tr> 
	    		...
	    		<th width="150">Corporate Customer ${show_direction('program_id')}</th>
			...
			%else:
				...
				%endif
					<td style="border-left:1px solid #ccc">
						%if h.status == -9:
							<a href="/sample/viewRequest?id=${h.id}" style="color: red;">${str(h)}</a>
						%elif h.status == -10:
							<a href="/sample/updateRequest?id=${h.id}&is_draft=true" style="color: black;">${str(h)}</a>
						%else:
							<a href="/sample/viewRequest?id=${h.id}">${str(h)}</a>
						%endif
						%if h.active == 1:
