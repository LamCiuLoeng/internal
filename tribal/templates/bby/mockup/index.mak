...
<div class="div-todolist">
  <table width="180" cellspacing="0" cellpadding="0" border="0">
    ...
    <tr>
      <td height="30" align="center" class="todoliset-menu" background="/images/TDLBG.jpg">Pending items(<span id="todo-new-count">${todolist["new_count"]}</span>)</td>
    </tr>
    <tr>
      <td>
	  	<ul id="todo-new">
	  		%for i,n in enumerate(todolist["new"]):
	  			%if i < 5 :
		  			<li><a href="/bbymockup/item_view?id=${n.id}">${n}</a>[${cd(n.update_time)}]</li>
		  		%else:
		  			<li class="more"><a href="/bbymockup/item_view?id=${n.id}">${n}</a>[${cd(n.update_time)}]</li>
		  		%endif
	  		%endfor
	  	</ul>
    ...
    <tr>
      <td height="30" align="center" class="todoliset-menu" background="/images/TDLBG.jpg">Sent items(<span id="todo-cancel-count">${todolist["send_count"]}</span>)</td>
    </tr>
    <tr>
      <td>
	  	<ul id="todo-send">
	  		%for i,s in enumerate(todolist["send"]):
	  			%if i < 5:
		  			<li><a href="/bbymockup/item_view?id=${s.id}">${s}</a>[${cd(s.update_time)}]</li>
		  		%else:
		  			<li><a href="/bbymockup/item_view?id=${s.id}">${s}</a>[${cd(s.update_time)}]</li>
		  		%endif
...
<div style="width:1200px;float:left">
	<div style="overflow:hidden;margin:5px 0px 10px 10px">
            ...
            ${widget(action="/bbymockup/index",value=values)|n}
		...
		<tbody>
			% for index,h in enumerate(result):
                ...
                %else:
                    <tr class="odd">
                %endif
				<td style="border-left:1px solid #ccc"><a href="/bbymockup/item_view?id=${h.id}">${h.sku}</a></td>
