...
<div id="function-menu">
  ...
  <tbody><tr>
  	...
  	<td width="64" valign="top" align="left"><a href="/sku/add"><img src="/images/images/menu_new_g.jpg"/></a></td>
...
<div class="div-todolist">
  <table width="180" cellspacing="0" cellpadding="0" border="0">
      ...
      <td>
	  	<ul id="todo-new">
	  	  %for i,n in enumerate(todolist["new"]):
	  	  	  %if i < 5:
	  	      	<li><a href="/sku/view?id=${n.id}">${n}</a> [${cd(n.update_time)}]</li>
	  	      %else:
	  	      	<li class="more"><a href="/sku/view?id=${n.id}">${n}</a> [${cd(n.update_time)}]</li>
	  	      %endif
	  	  %endfor
	  	</ul>
    ...
    <tr>
      <td height="30" align="center" class="todoliset-menu" background="/images/TDLBG.jpg">On hold(<span id="todo-cancel-count">${todolist["onhold_count"]}</span>)</td>
    </tr>
    <tr>
      <td>
	  	<ul id="todo-cancel">
	  	   %for i,o in enumerate(todolist["onhold"]):
	  	   	%if i < 5:
	  	        <li><a href="/sku/view?id=${o.id}">${o}</a> [${cd(o.update_time)}]</li>
	  	    %else:
	  	    	<li class="more"><a href="/sku/view?id=${o.id}">${o}</a> [${cd(o.update_time)}]</li>
	  	    %endif
	  	  %endfor
	  	</ul>
	  	%if todolist["onhold_count"] >5:
	  		<div class="more_div"><a href="#" onclick="showAndHide(this)"><span class="fn">More</span><span class="fn" style="display:none">Less</span></a></div>
	  	%endif
	  </td>
    </tr>
    <tr>
      <td height="30" align="center" class="todoliset-menu" background="/images/TDLBG.jpg">Canceled(<span id="todo-cancel-count">${todolist["cancel_count"]}</span>)</td>
    </tr>
    <tr>
      <td>
	  	<ul id="todo-cancel">
	  	  %for i,c in enumerate(todolist["cancel"]):
	  	  	%if i < 5:
	  	      	<li><a href="/sku/view?id=${c.id}">${c}</a> </li>
	  	    %else:
	  	    	<li><a href="/sku/view?id=${c.id}">${c}</a> </li>
	  	    %endif
	  	  %endfor
	  	</ul>
	  	%if todolist["cancel_count"] > 5:
	  		<div class="more_div"><a href="#" onclick="showAndHide()"><span class="fn">More</span><span class="fn" style="display:none">Less</span></a></div>
	  	%endif
	  </td>
    </tr>
  </tbody></table>
...
<div style="width:1500px;float:left">
    ...
    ${widget(action="/sku/index",value=values)|n}
  </div>
      ...
      <table cellspacing="0" cellpadding="0" border="0" id="dataTable" class="gridTable">
		<thead>
			...
			<tr> 
		    	  ...
		    	  <th width="150">Package Format</th>
		...
		<tbody>
			...
			%else:
				<tr class="odd">
			%endif
				<td style="border-left:1px solid #ccc"><a href="/sku/view?id=${h.id}">${h.sku}</a></td>
