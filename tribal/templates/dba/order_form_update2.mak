...
<script>
    ...
    var delItem = function(obj, itemCode){
    ...
    var cancelJob = function(po){
        if(confirm("Are you sure to cancel this order? \n " + po)){
            $("#save_form").attr("action", '/dba2/cancelOrder').submit();
        }
    }
...
<div id="function-menu">
  ...
  <tbody><tr>
    ...
    <td width="64" valign="top" align="left"><a href="/dba2/viewOrder?code=${rpacEncrypt(order.id)}"><img src="/images/images/menu_return_g.jpg"/></a></td>
    <td width="23" valign="top" align="left"><img height="21" width="23" src="/images/images/menu_last.jpg"/></td>
    <td valign="top" style="background:url(/images/images/menu_end.jpg) repeat-x;width:100%"></td>
  </tr>
</tbody></table>
</div>
<div class="nav-tree">PLAYTEX / WONDERBRA / SHOCK ABSORBER&nbsp;&nbsp;&gt;&nbsp;&nbsp;Order Revise</div>

<div class="clear"></div>
%if add_items or customer:
<div class="box2">
    <div class="toggle1"><a class="hid_a" href="javascript:void(0)" onclick="toggleIcon(this)"></a>Order Revise</div>
    <div style="margin-left:20px;">
        <p>
            <span class="STYLE3">Note:</span><br />
            <!--1)&nbsp;&nbsp;Please note that online ordering period is in between <span class="STYLE3">5th</span> and <span class="STYLE3">20th</span>.
            The system will close at <span class="STYLE3">20th midnight (HK time)</span> and re-open next month.
            <br />
    ...
    <div>
        <form id="save_form" action="/dba2/save_updateOrder" method="post">
        ...
        <table class="gridTable" cellpadding="0" cellspacing="0" border="0" >
            <thead>
                <tr>
                    <td colspan="100" style="border-right:0px;border-bottom:0px;">
                            ...
                            <div class="l">
                                <table class="gridTable" cellpadding="0" cellspacing="0" border="0" width="100%">
                                    <tr>
                                        ...
                                        <td style="border-top:1px solid #CCCCCC;">Customer PO#:</td>
                                        <td style="border-top:1px solid #CCCCCC;">${hidden_po}</td>
                                    </tr>
                                    <tr>
                                        <td style="border-left:1px solid #CCCCCC;"><span class="STYLE3">BILL TO</span>:</td>
                                        <td><textarea id="bill_to" name="bill_to" class="input-width">${order.bill_to}</textarea></td>
                                        <td><span class="STYLE3">SHIP TO</span>:</td>
                                        <td><textarea id="ship_to" name="ship_to" class="input-width">${order.ship_to}</textarea></td>
                                    </tr>
                                     ...
                                     <tr>
                                        ...
                                        <td><input id="sob" type="text" name="sob" style="width:96%;" value="${hidden_sob if hidden_sob else ''}" /></td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td><input id="ship_date" type="text" name="ship_date" value="${order.ship_date if order.ship_date else ''}" class="datePicker"/></td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                        <div class="l">
                            &nbsp;
                            <input type="button" id="btn_cancel_job" value="Cancel Order" class="btn" onclick="cancelJob('${hidden_po}')" />
                            &nbsp;&nbsp;
                            <input type="button" id="btn_save_to_job" value="Save Changes" class="btn" onclick="saveToJob()" />
                        </div>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th><span class="STYLE3">Qty (${Date2Text(order.create_time, '%b/%Y')})</span></th>
                    <th>Qty (Firm PO, ${Date2Text(order.create_time, '%b/%Y')})</th>
                    <th><span class="STYLE3">Forecast Qty (${nextMonth(order.create_time)})</span></th>
                    <th>Forecast Qty (For reference only, ${nextMonth(order.create_time)})</th>
                    <th>Item Code</th>
                    <th>Category</th>
                    <th>Type</th>
                    <th>Image</th>
            ...
            <tbody class="s_text">
               %for i in add_items:
               <%
                    from tribal.util.dba_util import returnQty
                    ...
                    if id_list and str(i.id) in id_list:
                        ...
                        qty_dict = returnQty(i.id, order.id)
                        qty1 = qty_dict.get('input_commited_qty', '')
                        qty2 = qty_dict.get('commited_qty', '')
                        qty3 = qty_dict.get('input_forecast_qty', '')
                        qty4 = qty_dict.get('forecast_qty', '')
               %>
                <tr>
                    <td class="head">
                        <input type="hidden" name="add_id" value="${i.id}" />
                        <input type="text" name="input_add_qty"  value="${qty1}" class="numeric" onblur="autoQty(this, 'actual_qty_', '${i.id}')"/>
                    </td>
                    <td class="head">
                        <input type="text" name="add_qty" value="${qty2}" autocomplete="off" readonly="readonly" readonly="readonly" class="readonly" id="actual_qty_${i.id}" />
                    </td>
                    <td class="head"><input name="input_forecast_qty" type="text" value="${qty3}"  class="numeric" onblur="autoQty(this, 'actual_forecast_qty_', '${i.id}')"/></td>
                    <td class="head">
                        <input type="text" name="forecast_qty" value="${qty4}" autocomplete="off" readonly="readonly" readonly="readonly" class="readonly" id="actual_forecast_qty_${i.id}"/>
                    </td>
                    <td>${i.item_code}</td>
                    ...
                    <td><a href="javascript:void(0)" onclick="javascript:delItem(this,'${i.item_code}')">Delete</a></td>
                </tr>
               %endfor
            </tbody>
        </table>
...
<div class="clear"></div>
<div class="box2">
    <form id="search_form" action="/dba2/updateOrder" method="post">
    ...
    <div>
        <table class="gridTable" cellpadding="0" cellspacing="0" border="0" style="width:74%">
            <thead>
                ...
                <tr>
                    <th class="head" style="width:5%;"><input type="checkbox" id="header_checkbox" value="0" onclick="selectAll(this)" /></th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="head" style="width:20px"><input type="checkbox" class="cboxClass" value="${i.id}" onclick="selectOne(this)" /></td>
                    <td>${i.item_code}</td>
                    <td>${i.category.name}</td>
                    <td>${i.type.name}</td>
                    <td>
